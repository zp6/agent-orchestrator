...
function makeStore(
  ...
  return {
    ...
    listActivePRGuardCooldowns: () => opts.cooldowns ?? [],
...
describe("/pr-guard-status command — active surge suppressions", () => {
  it("lists active suppressions with repo, issue, PR number, hit count, expiry, and remaining time", async () => {
      ...
      cooldowns: [
        {
          ...
          prNumber: 162,
          hitCount: 10,
          expiresAt: "2026-04-25T13:30:00.000Z",
        },
      ],
    });

    const reply = await runTelegramCommand(store, "/pr-guard-status");

    expect(reply).toContain("PR Guard Status");
    expect(reply).toContain("Active surge suppressions: 1");
    expect(reply).toContain("rapartlu/agent-proxy");
    expect(reply).toContain("issue #455");
    expect(reply).toContain("PR #162");
    expect(reply).toContain("hits 10");
    expect(reply).toContain("Suppression expires at: 2026-04-25 13:30 UTC");
    expect(reply).toContain("Time remaining:");
  });

  it("shows a no-suppression message when no active surge suppressions exist", async () => {
    const store = makeStore([]);
    const reply = await runTelegramCommand(store, "/pr-guard-status");

    expect(reply).toContain("No active surge suppressions");
  });
});
