...
// ── Q3 baseline snapshot types ────────────────────────────────────────────

/**
 * One Monday-aligned weekly bucket inside the May 2026 baseline snapshot.
 */
export interface Q3BaselineWeekPoint {
  /** ISO date of the week start (Monday). */
  week_start: string;
  /** Tasks completed successfully in this week. */
  tasks_completed: number;
  /** Tasks that failed or escalated in this week. */
  failed_tasks: number;
  /** All terminal tasks counted in this week. */
  total_tasks: number;
  /** Failed tasks / total tasks, or null when no tasks were recorded. */
  failure_rate: number | null;
  /** Total estimated USD spend across all task-linked LLM calls in this week. */
  total_cost_usd: number;
  /** Total spend divided by total tasks, or null when no tasks were recorded. */
  avg_cost_per_task_usd: number | null;
}

/**
 * Baseline snapshot for the May 2026 OKR measurements.
 */
export interface Q3BaselineSnapshot {
  generated_at: string;
  /** Baseline month in YYYY-MM format. */
  month: string;
  /** UTC timestamp at the start of the baseline month. */
  month_start: string;
  /** UTC timestamp at the start of the month after the baseline month. */
  month_end: string;
  /**
   * `ready` when the month has closed and real data was measured.
   * `pending` when the month is still in progress and the snapshot needs
   * to be re-run after the month closes.
   */
  status: "ready" | "pending";
  /** When `status = 'pending'`, rerun after this UTC timestamp. */
  re_run_after: string | null;
  /**
   * Average weekly completed tasks across the month. This is the baseline
   * value used to compare against the weekly_tasks_completed OKR.
   */
  weekly_tasks_completed: number | null;
  /** Failure rate across all terminal tasks in the month. */
  failure_rate: number | null;
  /** Average cost per terminal task in USD. */
  avg_cost_per_task_usd: number | null;
  /** Total terminal task count captured in the month. */
  total_tasks: number;
  /** Tasks that completed successfully in the month. */
  completed_tasks: number;
  /** Tasks that failed or escalated in the month. */
  failed_tasks: number;
  /** Total estimated USD spend across task-linked LLM calls in the month. */
  total_cost_usd: number | null;
  /** Monday-aligned weekly breakdown for traceability. */
  weekly_breakdown: Q3BaselineWeekPoint[];
  /** Human-readable note explaining pending/ready status. */
  note: string;
}
...
/**
 * Per-agent quality summary over a rolling approval window.
 * Used by the Telegram /quality-summary command and daily digest.
 */
export interface QualitySummaryAgentRow {
  agent_name: string;
  /** Approved tasks in the window for this agent. */
  total_approved: number;
  /** Approved tasks scoring below the threshold in the window. */
  below_threshold_count: number;
  /** Approved tasks with a non-null quality score in the window. */
  scored_approved: number;
  /** Mean quality score across scored approvals, or null when none exist. */
  avg_score: number | null;
  /** Lowest quality score across scored approvals, or null when none exist. */
  min_score: number | null;
}

/**
 * Rolling summary of approved task quality over the last N hours.
 */
export interface QualitySummaryReport {
  generated_at: string;
  /** Lookback window in hours. */
  window_hours: number;
  /** Score threshold for marginal approvals. */
  threshold: number;
  /** Total approved tasks in the window. */
  total_approved: number;
  /** Approved tasks with non-null scores in the window. */
  scored_approved: number;
  /** Approved tasks below the threshold in the window. */
  below_threshold_count: number;
  /**
   * Fraction of scored approvals below the threshold, or null when no scored
   * approvals exist in the window.
   */
  below_threshold_rate: number | null;
  /** Approved tasks in the window that still have no score. */
  unscored_approved: number;
  /** Per-agent rows, sorted by avg score ascending (worst first). */
  per_agent: QualitySummaryAgentRow[];
  /** The worst-scoring agent in the window, or null when no scored approvals exist. */
...
/**
 * Per-call-type and per-model token usage summary returned by `getTokenStats()`.
 */
...
export interface IStateStore {
  ...
  /**
   * Return a rolling approval-quality summary for the last `windowHours` hours.
   * Used by the Telegram /quality-summary command and daily digest.
   */
...
/** Active PR guard cooldown row as surfaced to operator status views. */
...
/** Optional metadata written alongside a PR guard cooldown row. */
export interface PRGuardCooldownMetadata {
  prNumber?: number | null;
  hitCount?: number | null;
}

/** Store interface used by the PR guard cooldown status feed. */
export interface IPRGuardCooldownFeedStore {
  listActivePRGuardCooldowns(repo?: string): PRGuardCooldownEntry[];
}
...
/**
 * Lifecycle status for a meeting synthesis signal stored in `state.db`.
 *
 * The orchestrator writes `meeting_synthesis_pending` when a meeting is
 * dispatched and later flips the row to `meeting_synthesis_resolved` once the
 * facilitator confirms the synthesis outcome.
 */
export type MeetingSynthesisSignalType =
  | "meeting_synthesis_pending"
  | "meeting_synthesis_resolved";

/**
 * A single meeting synthesis persistence row.
 *
 * Stored in the `meeting_synthesis_signals` table so the orchestrator can
 * recover from facilitator outages without losing the meeting audit trail.
 */
export interface MeetingSynthesisSignalRecord {
  id: number;
  meeting_id: string;
  topic: string;
  signal_type: MeetingSynthesisSignalType;
...
/**
 * Store interface for meeting synthesis persistence and reconciliation.
 *
 * Implemented by `StateStore`.  The orchestrator daemon uses these methods to
 * persist meeting dispatches locally, reconcile them on later cycles, and
 * record operator alerts when synthesis remains unresolved for too long.
 */
export interface IMeetingSynthesisStore {
  /**
   * Persist or refresh a pending meeting synthesis signal.
   *
   * Call this immediately after a meeting dispatch so the audit trail survives
   * a facilitator HTTP outage.
   */
  ...
  /**
   * Mark a pending meeting synthesis signal as resolved.
   *
  ...
  /**
   * Record that the orchestrator already alerted on a stale synthesis row.
   *
   * Returns `true` when an unresolved row was updated.
   */
  ...
  /**
   * Increment the retry counter for a meeting synthesis row.
   *
   * Used when the daemon re-attempts facilitator intake after a stale alert.
   */
  incrementMeetingSynthesisRetryCount(meetingId: string): boolean;

  /**
   * Return unresolved synthesis rows, newest first.
   */
  getPendingMeetingSynthesisSignals(limit?: number): MeetingSynthesisSignalRecord[];

  /**
   * Return unresolved synthesis rows older than the given age in hours.
   */
  getStaleMeetingSynthesisSignals(
    hours?: number,
    limit?: number,
...
export type SemanticMemoryEmbeddingSource = "ollama" | "cloud";

/**
 * Embedding metadata for a semantic-memory topic.
 */
export interface SemanticMemoryTopicEmbedding {
  topic: string;
...
/**
 * Result returned by the semantic-memory reindex job.
 */
export interface SemanticMemoryReindexReport {
  started_at: string;
  finished_at: string;
  total_topics: number;
  embedded_topics: number;
...
export interface ISemanticMemoryStore {
  ...
  /**
   * Return all distinct topics present in the semantic memory index.
   * Used by the daily reindex job.
   */
  listMemoryTopics(): string[];

  /**
   * Persist the embedding vector for a topic.
   * The vector is stored as JSON so it can be re-used by later reindex runs.
   */
  ...
  /**
   * Return the stored embedding metadata for all topics.
   * Used by semantic-memory background jobs and diagnostics.
   */
