...
export interface IStateStore {
  ...
  // Security false-positive exemptions
  ...
  /** Write or refresh many cooldowns for the same repo in one call. */
  setPRGuardCooldowns?(repo: string, issueNumbers: number[], ttlMinutes?: number): void;
  /** Return the expiry timestamp for an active cooldown, or null if none exists. */
  ...
  /** Return all active cooldowns, optionally filtered by repo. */
...
export interface VerificationResultRecord {
   ...
   *
   * Phase-gate dispatcher refusals may also use `'phase_a_gate'` to indicate
   * that the task was blocked before scoring by the orchestrator.
   */
