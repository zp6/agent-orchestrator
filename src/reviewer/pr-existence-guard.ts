...
import { extractClosesRefs } from "./pr-scope-checker.js";
...
function getPRCoverageIssueNumbers(pr: OpenPRSummary, matchedIssueNumber: number): number[] {
  const issueNumbers = new Set<number>([matchedIssueNumber]);
  for (const issueNumber of extractClosesRefs(pr.body)) {
...
export async function checkPRExistenceBeforeDispatch(
  ...
  try {
    ...
    if (match) {
      const coveredIssueNumbers = getPRCoverageIssueNumbers(match, issueNumber);
      const coverageLabel =
        coveredIssueNumbers.length > 1
          ? `PR #${match.number} covers ${coveredIssueNumbers.length} issues`
          : `Issue #${issueNumber} already has open PR #${match.number}`;
      const reason =
        coveredIssueNumbers.length > 1
          ? `${coverageLabel} (${coveredIssueNumbers.map((n) => `#${n}`).join(", ")}) — ` +
            `routing to review queue instead of re-dispatching`
          : `Issue #${issueNumber} already has open PR #${match.number} ` +
      ...
      log.info("Existing PR found — skipping re-dispatch", {
        ...
        coveredIssueCount: coveredIssueNumbers.length,
        coveredIssueNumbers,
      });
      ...
      if (opts?.cooldownStore) {
        try {
          ...
          if (opts.cooldownStore.setPRGuardCooldowns) {
            opts.cooldownStore.setPRGuardCooldowns(repo, coveredIssueNumbers, ttl);
          } else {
            for (const coveredIssueNumber of coveredIssueNumbers) {
              opts.cooldownStore.setPRGuardCooldown(repo, coveredIssueNumber, ttl);
            }
          }
