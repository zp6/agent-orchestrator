 ...
 *   /pr-guard-status       → active PR guard surge suppressions and remaining TTL
 ...
 *   /token-stats [hours] → per-call-type + per-model LLM token usage (default: 720h = 30 days)
 ...
 *   /quality-summary [hours] [threshold] → rolling approval-quality summary with marginal rate and worst agent
...
import { findSimilarMemoryTopics } from "../orchestrator/semantic-memory.js";
import {
...
async function executeCommand(
  ...
  switch (cmd.command) {
    ...
    case "pr-guard-status":
      return handlePRGuardStatus(store);
    ...
    case "quality-summary": {
      const hours = parseInt(cmd.args[0] ?? "24", 10);
      const windowHours = Number.isNaN(hours) || hours < 1 ? 24 : Math.min(hours, 720);
      const thresholdArg = cmd.args[1]?.trim();
      const threshold = thresholdArg
        ? Math.min(Math.max(parseFloat(thresholdArg) || 0.8, 0.01), 1.0)
        : 0.8;
      return handleQualitySummary(store, windowHours, threshold, dashboardUrl);
    }
...
function handlePRGuardStatus(store: ITelegramStateStore): string {
  const payload = getPRGuardCooldownFeedPayload(store);
  const activeSuppressionRows = payload.cooldowns.filter(
    (entry) => entry.hitCount != null && entry.prNumber != null,
  );

  if (activeSuppressionRows.length === 0) {
    return [
      `🛡️ *PR Guard Status*`,
      ``,
      `No active surge suppressions.`,
      `The cooldown table has no rows currently written by the PR guard surge detector.`,
    ].join("\n");
  }
  ...
  const lines: string[] = [
    `🛡️ *PR Guard Status*`,
    ``,
    `Active surge suppressions: ${activeSuppressionRows.length}`,
  ];

  activeSuppressionRows.forEach((entry, index) => {
    lines.push(``);
    lines.push(`${index + 1}. \`${entry.repo}\` · issue #${entry.issueNumber}`);
    lines.push(`   PR #${entry.prNumber ?? "unknown"} · hits ${entry.hitCount ?? "unknown"}`);
    lines.push(`   Suppression expires at: ${formatIsoUtc(entry.expiresAt)}`);
    lines.push(`   Time remaining: ${formatTimeRemaining(entry.timeRemainingSeconds)}`);
  });

  return lines.join("\n");
}

function formatIsoUtc(timestamp: string): string {
...
function handleTokenStats(store: ITelegramStateStore, sinceHours: number): string {
  ...
  for (const r of rows) {
    ...
    lines.push(
      `\`${r.call_type}\` / \`${r.model}\` — ${r.call_count} calls`,
...
// ── Rolling quality summary handler (issue #490) ──────────────────────────
...
function handleQualitySummary(
  ...
  const report: QualitySummaryReport = buildQualitySummaryReport(store, {
    windowHours,
    threshold,
  });
  return formatQualitySummaryForTelegram(report, { dashboardUrl });
}
 ...
 */
async function handleMemoryExpand(store: ITelegramStateStore, topic: string): Promise<string> {
  const memoryStore = store as unknown as ISemanticMemoryStore;
  ...
  if (entries.length === 0) {
    if (typeof memoryStore.listMemoryTopicEmbeddings === "function") {
      try {
        const similar = await findSimilarMemoryTopics(
          topic,
          memoryStore as ISemanticMemoryStore & {
            listMemoryTopicEmbeddings(): {
              topic: string;
              embedding_json: string;
              embedding_model: string;
              embedding_source: "ollama" | "cloud";
              embedded_at: string;
            }[];
          },
          { limit: 1 },
        ...
        if (best) {
          const bestEntries = memoryStore.expandMemoryTopic(best.topic, 15);
          if (bestEntries.length > 0) {
            const lines: string[] = [
              `🧠 *Memory Expand: \`${topic}\`*`,
              ``,
              `No exact or FTS match found. Closest indexed topic: \`${best.topic}\` (${(best.similarity * 100).toFixed(0)}% similarity)`,
              ``,
            ];
        ...
        // Embedding lookup is best-effort only; fall through to the standard
        // no-results response if the local model is unavailable.
      }
    }
